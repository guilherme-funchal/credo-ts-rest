...
describe('Utils | Credentials', () => {
    describe('convertAttributesToCredentialValues', () => {
        test('returns object with raw and encoded attributes', () => {
            ...
            expect((0, credential_1.convertAttributesToCredentialValues)(attributes)).toEqual({
                name: {
                    raw: '101 Wilson Lane',
                    encoded: '68086943237164982734333428280784300550565381723532936263016368251445461241953',
                },
    ...
    describe('assertCredentialValuesMatch', () => {
        test('does not throw if attributes match', () => {
            ...
            expect(() => (0, credential_1.assertCredentialValuesMatch)(firstValues, secondValues)).not.toThrow();
        });
        test('throws if number of values in the entries do not match', () => {
            ...
            expect(() => (0, credential_1.assertCredentialValuesMatch)(firstValues, secondValues)).toThrow('Number of values in first entry (1) does not match number of values in second entry (2)');
        });
        test('throws if second value does not contain key from first value', () => {
            ...
            expect(() => (0, credential_1.assertCredentialValuesMatch)(firstValues, secondValues)).toThrow("Second cred values object has no value for key 'name'");
        });
        test('throws if encoded values do not match', () => {
            ...
            expect(() => (0, credential_1.assertCredentialValuesMatch)(firstValues, secondValues)).toThrow("Encoded credential values for key 'age' do not match");
        });
        test('throws if raw values do not match', () => {
            ...
            expect(() => (0, credential_1.assertCredentialValuesMatch)(firstValues, secondValues)).toThrow("Raw credential values for key 'age' do not match");
        });
    });
    describe('checkValidEncoding', () => {
        ...
        test.each(testEntries)('returns true for valid encoding %s', (_, raw, encoded) => {
            expect((0, credential_1.checkValidCredentialValueEncoding)(raw, encoded)).toEqual(true);
        });
    });
});
