...
function encodeCredentialValue(value) {
    ...
    return new bn_js_1.default(core_1.Hasher.hash(core_1.Buffer.from(value), 'sha2-256')).toString();
}
function assertAttributesMatch(schema, attributes) {
    ...
    if (difference.length > 0) {
        throw new core_1.AriesFrameworkError(`The credential preview attributes do not match the schema attributes (difference is: ${difference}, needs: ${schemaAttributes})`);
    }
}
/**
...
function createAndLinkAttachmentsToPreview(attachments, previewAttributes) {
    ...
    attachments.forEach((linkedAttachment) => {
        if (credentialPreviewAttributeNames.includes(linkedAttachment.attributeName)) {
            throw new core_1.AriesFrameworkError(`linkedAttachment ${linkedAttachment.attributeName} already exists in the preview`);
        }
        else {
