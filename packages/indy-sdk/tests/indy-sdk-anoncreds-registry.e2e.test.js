...
const IndySdkAnonCredsRegistry_1 = require("../src/anoncreds/services/IndySdkAnonCredsRegistry");
const ledger_1 = require("../src/ledger");
const anoncreds_1 = require("./__fixtures__/anoncreds");
const setupIndySdkModule_1 = require("./setupIndySdkModule");
const agentConfig = (0, helpers_1.getAgentConfig)('IndySdkAnonCredsRegistry');
const indySdkModules = (0, setupIndySdkModule_1.getIndySdkModules)();
const agent = new core_1.Agent({
    config: agentConfig,
    dependencies: helpers_1.agentDependencies,
    modules: indySdkModules,
});
const indySdkAnonCredsRegistry = new IndySdkAnonCredsRegistry_1.IndySdkAnonCredsRegistry();
const indySdkPoolService = agent.dependencyManager.resolve(ledger_1.IndySdkPoolService);
...
describe('IndySdkAnonCredsRegistry', () => {
    beforeAll(() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        yield (0, helpers_1.importExistingIndyDidFromPrivateKey)(agent, core_1.TypedArrayEncoder.fromString(helpers_1.publicDidSeed));
    }));
    ...
    test('register and resolve a schema and credential definition', () => __awaiter(void 0, void 0, void 0, function* () {
        const dynamicVersion = `1.${Math.random() * 100}`;
        const legacyIssuerId = 'TL1EaPFCZ8Si5aUrqScBDt';
        const signingKey = core_1.Key.fromPublicKeyBase58('FMGcFuU3QwAQLywxvmEnSorQT3NwU9wgDMMTaDFtvswm', core_1.KeyType.Ed25519);
        ...
        const schemaResult = yield indySdkAnonCredsRegistry.registerSchema(agent.context, {
            schema: {
                attrNames: ['name'],
        ...
        const legacySchema = yield indySdkAnonCredsRegistry.getSchema(agent.context, legacySchemaId);
        ...
        const didIndySchema = yield indySdkAnonCredsRegistry.getSchema(agent.context, didIndySchemaId);
        ...
        const credentialDefinitionResult = yield indySdkAnonCredsRegistry.registerCredentialDefinition(agent.context, {
            credentialDefinition: {
                issuerId: didIndyIssuerId,
                tag: 'TAG',
                schemaId: didIndySchemaId,
                type: 'CL',
                value: anoncreds_1.credentialDefinitionValue,
            },
            options: {},
        });
        ...
        const legacyCredentialDefinition = yield indySdkAnonCredsRegistry.getCredentialDefinition(agent.context, legacyCredentialDefinitionId);
        ...
        const didIndyCredentialDefinition = yield indySdkAnonCredsRegistry.getCredentialDefinition(agent.context, didIndyCredentialDefinitionId);
        ...
        const revocationRegistryRequest = yield setupIndySdkModule_1.indySdk.buildRevocRegDefRequest('TL1EaPFCZ8Si5aUrqScBDt', {
        ...
        yield indySdkPoolService.submitWriteRequest(agent.context, pool, revocationRegistryRequest, signingKey);
        // indySdk.buildRevRegEntry panics, so we just pass a custom request directly
        const entryResponse = yield indySdkPoolService.submitWriteRequest(agent.context, pool, {
        ...
        const legacyRevocationRegistryDefinition = yield indySdkAnonCredsRegistry.getRevocationRegistryDefinition(agent.context, legacyRevocationRegistryId);
        ...
        const didIndyRevocationRegistryDefinition = yield indySdkAnonCredsRegistry.getRevocationRegistryDefinition(agent.context, didIndyRevocationRegistryId);
        ...
        const legacyRevocationStatusList = yield indySdkAnonCredsRegistry.getRevocationStatusList(agent.context, legacyRevocationRegistryId, entryResponse.result.txnMetadata.txnTime);
        ...
        const didIndyRevocationStatusList = yield indySdkAnonCredsRegistry.getRevocationStatusList(agent.context, didIndyRevocationRegistryId, entryResponse.result.txnMetadata.txnTime);
