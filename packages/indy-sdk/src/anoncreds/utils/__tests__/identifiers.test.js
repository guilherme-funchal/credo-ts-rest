...
describe('identifiers', () => {
    describe('indySdkAnonCredsRegistryIdentifierRegex', () => {
        test('matches against a legacy schema id, credential definition id and revocation registry id', () => {
            ...
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(did)).toEqual(false);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(schemaId)).toEqual(true);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(credentialDefinitionId)).toEqual(true);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(revocationRegistryId)).toEqual(true);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(anotherId)).toEqual(false);
        });
        test('matches against a did indy did, schema id, credential definition id and revocation registry id', () => {
            ...
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(did)).toEqual(true);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(schemaId)).toEqual(true);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(credentialDefinitionId)).toEqual(true);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(revocationRegistryId)).toEqual(true);
            expect(identifiers_1.indySdkAnonCredsRegistryIdentifierRegex.test(anotherId)).toEqual(false);
