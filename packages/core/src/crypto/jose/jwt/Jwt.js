...
class Jwt {
    ...
    static fromSerializedJwt(serializedJwt) {
        if (typeof serializedJwt !== 'string' || !Jwt.format.test(serializedJwt)) {
            throw new error_1.AriesFrameworkError(`Invalid JWT. '${serializedJwt}' does not match JWT regex`);
        }
        ...
        try {
            ...
            throw new error_1.AriesFrameworkError(`Invalid JWT. ${error instanceof Error ? error.message : JSON.stringify(error)}`);
        }
    }
}
exports.Jwt = Jwt;
